...
module("Integration | Component | header", function (hooks) {
  ...
  test("it renders", async function (assert) {
    ...
    assert.dom().hasTextContaining("Test Title");
  });
  ...
  test("it renders with Wandertext icon and title with no attributes set", async function (assert) {
    ...
    assert
      .dom("[data-test-header-center]")
      .hasTextContaining("Wandertext Logo");
  });
  ...
  test("it renders with place icon and random title when set", async function (assert) {
    ...
    assert.dom("h1").hasText(fakeTitle);
    assert.dom("[data-test-header-center]").hasTextContaining("Place Icon");
  });
